...
async function updateRule(item) {
    await chrome.runtime.sendMessage({message: "updateRules", data: item});
}

async function updateCSS(rule){
    cssRules +=
`
${rule.selection}{
    ${rule.style}
...
async function init() {
    ...
    */
    let rules = (await chrome.runtime.sendMessage({message: "giveData"})).rules
    console.log(rules)
    rules.forEach((rule) => {
        ...
        function (request, sender) {
            ...
            function HighlightSVG(elementArray) {
                ...
                elementArray.forEach((element, i) => {
                    ...
                    div.innerHTML = `
                <svg id="svg${i}" style="" xmlns="http://www.w3.org/2000/svg"  class="highlight_svgaefefef">
            ...
            `
                ...
                buttons.delete.addEventListener("click", () => {
                    ...
                    function PreciseSelection(el) {
                        ...
                        //pseudo-classes now
                        let pseudo = [":empty", ":only-child", ":only-of-type", ":first-child", ":last-child", ":first-of-type", ":last-of-type"]
